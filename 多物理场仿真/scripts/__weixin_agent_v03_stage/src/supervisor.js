...
const OUTLOOK_FILE = path.join(SCRIPT_DIR, "outlook-worker.js");
...
const OUTLOOK_TOKEN_FILE = path.join(STATE_DIR, "outlook", "token.dpapi");
...
let outlookChild = null;
let outlookRestartAfter = 0;
let stopping = false;
...
function reconcile() {
  ...
  const outlookDisabled = ["0", "false", "off", "no"].includes(
    String(process.env.OUTLOOK_ENABLED || "true").toLowerCase(),
  );
  const shouldRunOutlook = !outlookDisabled && fs.existsSync(OUTLOOK_TOKEN_FILE);
  if (shouldRunOutlook && !outlookChild && outlookRestartAfter <= Date.now()) {
    outlookChild = spawn(
      process.execPath,
      ["--disable-warning=ExperimentalWarning", OUTLOOK_FILE],
      { cwd: PROJECT_DIR, env: { ...process.env }, windowsHide: true, stdio: "ignore" },
    );
    log(`Outlook 邮件监听已启动，PID=${outlookChild.pid}`);
    outlookChild.once("error", (error) => {
      log(`Outlook 监听启动异常：${String(error).slice(0, 500)}`);
    });
    outlookChild.once("exit", (code, signal) => {
      outlookChild = null;
      if (!stopping) {
        outlookRestartAfter = Date.now() + 30_000;
        log(`Outlook 监听已退出 code=${code} signal=${signal || "-"}，30 秒后重试`);
      }
    });
  } else if (!shouldRunOutlook && outlookChild) {
    outlookChild.kill();
  }
}
...
async function shutdown(signal) {
  ...
  stopping = true;
  clearInterval(timer);
  log(`收到 ${signal}，正在停止 ${children.size} 个微信账号和后台服务`);
  const exits = [];
  ...
  if (outlookChild) {
    exits.push(new Promise((resolve) => outlookChild.once("exit", resolve)));
    outlookChild.kill();
  }
  await Promise.race([
