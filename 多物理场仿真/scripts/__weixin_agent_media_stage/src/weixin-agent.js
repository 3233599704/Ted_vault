...
import { CustomStickerLibrary } from "./sticker-library.js";
...
import { StickerTool } from "./tools/sticker-tool.js";
...
async function start() {
  ...
  const weixin = new WeixinApi(CONFIG, log);
  const mediaProcessor = new MediaProcessor(CONFIG, { log });
  const stickerLibrary = new CustomStickerLibrary(CONFIG);
  const toolRegistry = new ToolRegistry([
    new CodexDiagnosticTool(CONFIG),
    new ReminderTool(CONFIG),
    new DouyinTool(CONFIG, { log }),
    new StickerTool(CONFIG),
  ...
  log(`工具层: 抖音总结 ${CONFIG.mimoVideoModel} / 股票公开数据 / Codex ${CONFIG.codexEnabled ? "只读诊断" : "关闭"}`);
  ...
  if (CONFIG.codexEnabled && !CONFIG.codexAdminUsers.size) log("安全提醒: Codex 已启用但没有配置管理员，诊断入口不可用");
    ...
    while (running) {
      ...
      try {
        ...
        for (const message of response.msgs || []) {
          ...
          if (!from || (!text && !inbound.media.length)) {
            log(`跳过不支持的消息: from=${from || "unknown"} key=${key}`);
            continue;
          }
          ...
          const mediaLabel = inbound.media.map((item) => item.type).join(",");
          log(
            `收到微信消息 ${from.slice(-12)}: ` +
              `${text.slice(0, 120).replace(/\n/g, " ") || `[${mediaLabel}]`}`,
          );

          if (!inbound.media.length && text.trim().toLowerCase() === "/cancel") {
          ...
          const burst = storage.enqueueInboundBurst({
            id: inboundJobId(key),
            messageKey: key,
            userId: from,
            payload: { text, media: inbound.media, contextToken, receivedAt: Date.now() },
            quietMs: CONFIG.messageBurstQuietMs,
            maxWaitMs: CONFIG.messageBurstMaxWaitMs,
            maxMessages: CONFIG.messageBurstMaxMessages,
            notBefore: modelReadyAt,
          });
          if (burst.inserted) {
            log(
              burst.merged
                ? `消息已合并到待回复批次: ${burst.messageCount} 条 / ${burst.jobId.slice(-16)}`
                : `消息已进入待回复批次: ${key.slice(-16)}`,
            );
          }
        }
