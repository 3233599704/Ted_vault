...
function textWithReference(item) {
...
export function extractInboundContent(message) {
  ...
  for (const item of message.item_list || []) {
    if (item.type === ITEM_TEXT && item.text_item?.text != null) {
      const text = textWithReference(item);
    ...
    if (item.type === ITEM_IMAGE && item.image_item?.media) {
      media.push({
        type: "image",
        media: mediaReference(item.image_item.media),
        imageAesKey: item.image_item.aeskey || "",
      });
      continue;
    }
    if (item.type === ITEM_VOICE && item.voice_item) {
      ...
      if (transcript) {
        ...
        media.push({
          type: "voice",
          media: mediaReference(item.voice_item.media),
          encodeType: Number(item.voice_item.encode_type || 0),
          sampleRate: Number(item.voice_item.sample_rate || 0),
          playtime: Number(item.voice_item.playtime || 0),
        });
      }
    }
  }
...
export function extractMessageText(message) {
  return extractInboundContent(message).text;
}
...
export class WeixinApi {
  ...
  async sendText(account, to, text, contextToken, deliveryId) {
    ...
    for (let index = 0; index < chunks.length; index += 1) {
      ...
      assertSuccessfulSend(response, "sendMessage");
    }
  }

  async sendItem(account, to, item, contextToken, deliveryId, suffix = "item") {
    const stableId = crypto
      .createHash("sha256")
      .update(`${deliveryId}:${suffix}`)
      .digest("hex")
      .slice(0, 28);
    const response = await this.request({
      method: "POST",
    ...
    assertSuccessfulSend(response, "sendMediaMessage");
  }

  async sendImageFile(account, to, filePath, contextToken, deliveryId, caption = "") {
    const buffer = fs.readFileSync(filePath);
    if (buffer.length > this.config.weixinOutboundImageMaxBytes) {
      const error = new Error(`表情包图片过大: ${buffer.length} bytes`);
      error.retryable = false;
      throw error;
    }
    const mime = detectImageMime(buffer);
    if (mime === "application/octet-stream") {
      const error = new Error("表情包必须是 JPG、PNG、GIF 或 WebP");
      error.retryable = false;
      throw error;
    }
    const prepared = prepareEncryptedUpload(buffer);
    ...
    const uploadUrl = assertAllowedUploadUrl(buildCdnUploadUrl({
      uploadFullUrl: upload.upload_full_url?.trim(),
      uploadParam: upload.upload_param,
      filekey: prepared.filekey,
      cdnBaseUrl: this.config.weixinCdnBaseUrl,
    }), this.config.weixinCdnBaseUrl);
    const downloadParam = await uploadEncryptedBuffer({
      url: uploadUrl,
      ciphertext: prepared.ciphertext,
      fetchImpl: this.fetchImpl,
      timeoutMs: this.config.weixinMediaTimeoutMs,
    });
    if (caption) await this.sendText(account, to, caption, contextToken, `${deliveryId}:caption`);
    await this.sendItem(account, to, {
      type: ITEM_IMAGE,
      image_item: {
        media: {
          encrypt_query_param: downloadParam,
          aes_key: Buffer.from(prepared.aesKeyHex, "utf8").toString("base64"),
          encrypt_type: 1,
        },
        mid_size: prepared.filesize,
      },
    }, contextToken, deliveryId, "image");
  }
