...
export function splitChatMessages(text) {
...
export class JobWorker {
  constructor({ config, storage, agent, weixin, account, log, toolRegistry = null, mediaProcessor = null, stickerLibrary = null, kinds = [], name = "main", recoverOnStart = true }) {
    this.config = config;
  ...
  async process(job) {
    try {
      ...
      else if (job.kind === "tool" || job.kind === "codex") await this.processTool(job);
      ...
      if (error?.retryable !== false && job.attempts < this.config.maxJobAttempts) {
        ...
        if (job.kind === "tool" || job.kind === "codex") this.enqueueToolFailure(job, error);
  ...
  async processOutbound(job) {
    const { to, text = "", imagePath = "", contextToken, refreshContext = false } = job.payload;
    const activeContext = refreshContext
      ? this.storage.getLatestContext(job.user_id)?.context_token || contextToken
      : contextToken;
    if (imagePath) {
      await this.weixin.sendImageFile(this.account, to, imagePath, activeContext, job.id, text);
      this.log(`图片消息发送成功: ${to.slice(-12)} / ${imagePath}`);
      return;
    }
    await this.weixin.sendText(this.account, to, text, activeContext, job.id);
  ...
  commandReply(job, text) {
    ...
    if (lower === "/help") {
      return [
        ...
        "/stock picks 生成模拟盘候选和分批数量",
        "/paper capital 100000 设置模拟本金",
        "/paper buy 600519 100 1500 记录模拟买入",
        "/paper sell 600519 100 1600 记录模拟卖出",
        "/paper portfolio 检查持仓和卖出信号",
        "/sticker list 查看表情，/sticker love 发送",
        "/cancel 取消正在生成的回复",
      ].join("\n");
    }
  ...
  async processInbound(job) {
    const { text = "", contextToken, media = [] } = job.payload;
    let result = job.result || {};

    await this.weixin.sendTyping(this.account, job.user_id, contextToken, 1);
    try {
      if (!result.mediaInput && media.length) {
        if (!this.mediaProcessor) throw new Error("媒体处理器未配置");
        const mediaInput = await this.mediaProcessor.process({ text, media });
        result = { ...result, mediaInput };
        this.storage.saveJobCheckpointWithUsage(
          job.id,
          result,
          job.user_id,
          mediaInput.usageEvents,
        );
      }
      if (!result?.reply && !result?.stickerIntent) {
        const routeText = result.mediaInput?.routeText || text;
        const modelText = result.mediaInput?.modelText || text;
        let reply = this.commandReply(job, routeText);
        let model = "local";
        let usage = {};
        let latencyMs = 0;
        let replyKind = "local";
        let stickerIntent = "";
        if (!reply) {
          const route = this.toolRegistry?.route(routeText, {
            userId: job.user_id,
            history: this.storage.getHistory(job.user_id, 16, 18_000),
          });
          if (route) {
            const jobKind = route.jobKind || "tool";
            const toolId = `${jobKind}:${job.id}`;
            this.storage.enqueueJob({
              id: toolId,
              kind: jobKind,
              userId: job.user_id,
              ...
              payload: {
                ...
                action: route.action,
                args: route.args || {},
                originalText: routeText,
                contextToken,
              },
            });
            if (route.silentAcknowledgement) {
              this.storage.saveJobResult(job.id, {
                ...result,
                reply: "",
                model: "local:silent-tool-route",
                usage: {},
                latencyMs: 0,
                silent: true,
              });
              this.log(`静默转入后台任务: ${route.tool} / ${job.user_id.slice(-12)}`);
              return;
            }
        ...
        if (!reply) {
          ...
          stickerIntent = response.stickerIntent || "";
          replyKind = "chat";
        }
        result = { ...result, reply, model, usage, latencyMs, replyKind, stickerIntent };
        this.storage.saveJobResult(job.id, result);
      }

      if (result.replyKind === "chat") {
        const messages = splitChatMessages(result.reply);
        const sentMessages = Math.min(
          Math.max(Number(result.sentMessages || 0), 0),
          messages.length,
        );
        for (let index = sentMessages; index < messages.length; index += 1) {
          ...
          result = { ...result, sentMessages: index + 1 };
          this.storage.saveJobResult(job.id, result);
        }
        if (result.stickerIntent && !result.stickerSent && this.stickerLibrary) {
          try {
            const sticker = await this.stickerLibrary.prepare(result.stickerIntent, job.id);
            if (sticker?.preparedPath) {
              await this.weixin.sendImageFile(
                this.account,
                job.user_id,
                sticker.preparedPath,
                contextToken,
                `${job.id}:custom-sticker`,
              );
              this.log(`自定义表情发送成功: ${result.stickerIntent} / ${sticker.file}`);
            }
            result = { ...result, stickerSent: true };
          } catch (error) {
            result = { ...result, stickerSent: true, stickerError: String(error).slice(0, 500) };
            this.log(`自定义表情跳过: ${result.stickerIntent} / ${String(error).slice(0, 300)}`);
          }
          this.storage.saveJobResult(job.id, result);
        }
      } else {
  ...
  async processTool(job) {
    ...
    if (!result) {
      ...
      result = await this.toolRegistry.run(job.payload.tool, { job, storage: this.storage });
      result.latencyMs = Number(result.latencyMs || Date.now() - started);
      if (result.skipped || (!result.reply && !result.imagePath)) {
        this.storage.saveJobResult(job.id, result);
      } else {
        this.storage.saveToolResultAndTurn(job.id, result, {
          userId: job.user_id,
          userText: job.payload.originalText || `[后台任务: ${job.payload.tool}]`,
          assistantText: result.reply || `[发送表情包 ${result.stickerId || ""}]`,
